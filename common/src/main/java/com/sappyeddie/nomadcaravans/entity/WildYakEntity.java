...
public class WildYakEntity extends Animal implements GeoEntity {

    private static final EntityDataAccessor<Boolean> DATA_HAS_CHEST =
            SynchedEntityData.defineId(WildYakEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    private static final EntityDataAccessor<Byte> DATA_CARPET_COLOR =
            SynchedEntityData.defineId(WildYakEntity.class, EntityDataSerializers.BYTE);
    ...
    private static final EntityDataAccessor<String> DATA_OWNER_UUID =
            SynchedEntityData.defineId(WildYakEntity.class, EntityDataSerializers.STRING);
    ...
    private static boolean isHayBale(ItemStack stack) {
        return stack.is(Items.HAY_BLOCK);
    }

    private static ItemStack createDeed(WildYakEntity yak) {
        ItemStack deed = new ItemStack(ModRegistries.YAK_DEED.get());
        UUID yakId = yak.getUUID();
        CompoundTag tag = new CompoundTag();
        tag.putLong(DEED_YAK_UUID_MOST_KEY, yakId.getMostSignificantBits());
        tag.putLong(DEED_YAK_UUID_LEAST_KEY, yakId.getLeastSignificantBits());
    ...
    public WildYakEntity(EntityType<? extends WildYakEntity> entityType, Level level) {
    ...
    public DyeColor getCarpetColor() {
        byte id = this.entityData.get(DATA_CARPET_COLOR);
        return id >= 0 ? DyeColor.byId(id) : null;
    }
    ...
    protected void readAdditionalSaveData(ValueInput input) {
        ...
        this.setChest(input.getBooleanOr("HasChest", false));
    ...
    public class YakGrazeGoal extends Goal {
        ...
        private final WildYakEntity yak;
        private int eatTime;

        public YakGrazeGoal(WildYakEntity yak) {
            this.yak = yak;
        }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (isHayBale(stack) && !this.isTamed()) {
            if (!this.level().isClientSide()) {
                this.setOwnerUUID(player.getUUID());
                stack.consume(1, player);
                showTamedHearts();
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        controllers.add(new AnimationController<WildYakEntity>("reaction", 0, state -> {
    ...
    public @Nullable AgeableMob getBreedOffspring(ServerLevel level, AgeableMob partner) {
        ...
        if (baby != null) {
            ...
            if (partner instanceof WildYakEntity yakPartner) {
                UUID thisOwner = this.getOwnerUUID();
                if (thisOwner != null && thisOwner.equals(yakPartner.getOwnerUUID())) {
